...
// ===== PERLIN-NOISE FUNKTIONEN =====
// Implementierung von Perlin-Noise für Terrain-Generierung

// Simplex-Noise ähnliche Implementierung für 2D
float fade(float t) {
...
float perlinNoise2D(float x, float y) {
    int X = (int)llFloor(x) & 255;
    int Y = (int)llFloor(y) & 255;
    ...
    // Simulierte Hash-Tabelle für Determinismus
    integer hash1 = (X + Y * 31) % 256;
    integer hash2 = (X + 1 + Y * 31) % 256;
    integer hash3 = (X + (Y + 1) * 31) % 256;
    integer hash4 = (X + 1 + (Y + 1) * 31) % 256;
    
    float a = grad(hash1, x, y);
    float b = grad(hash2, x - 1, y);
    float c = grad(hash3, x, y - 1);
    float d = grad(hash4, x - 1, y - 1);
    
    return lerp(lerp(a, b, u), lerp(c, d, u), v);
}

// Erweiterte Terrain-Generierung mit Perlin-Noise
float generateTerrainHeight(float x, float y) {
    float height = 0.0;
    float amplitude = NOISE_AMPLITUDE;
    float frequency = 1.0 / NOISE_SCALE;
...
// ===== FUNKTION ZUM EINEBNEN DER REGION MIT PERLIN-NOISE =====
flattenRegion(float targetHeight)
{
    ...
    llOwnerSay("🏔️ Region wird mit Perlin-Noise auf " + (string)targetHeight + " Meter generiert...");
    ...
    for (x = 0; x < regionX; x += blockSize)
    {
        for (y = 0; y < regionY; y += blockSize)
        {
            ...
            for (ix = x; ix < endX; ix++)
            {
                for (iy = y; iy < endY; iy++)
                {
                    // Perlin-Noise für natürlichere Landschaft
                    float height;
                    if (targetHeight == FLATTEN_HEIGHT) {
                        // Bei Einebnen: Perlin-Noise mit geringer Amplitude für sanfte Hügel
                        height = targetHeight + perlinNoise2D(ix / NOISE_SCALE, iy / NOISE_SCALE) * 1.5;
                    } else {
                        height = targetHeight;
                    }
                    osSetTerrainHeight(ix, iy, height);
                }
            }
            llSleep(0.01);
        }
    ...
    osTerrainFlush();
    llOwnerSay("✅ Region wurde mit Perlin-Noise generiert!");
}

// ===== FUNKTION ZUM AUFRAUHEN DER REGION MIT PERLIN-NOISE =====
roughenRegion(float roughnessAmount, float maxHeightChange)
{
    ...
    llOwnerSay("🏔️ Region wird mit Perlin-Noise aufgeraut (Stärke: " + (string)(roughnessAmount * 100) + "%, max. Höhenänderung: " + (string)maxHeightChange + "m)...");
        ...
        for (y = 0; y < regionY; y += blockSize)
        {
            ...
            for (ix = x; ix < endX; ix++)
            {
                for (iy = y; iy < endY; iy++)
                {
                    ...
                    // Kombination von Perlin-Noise und Zufall für realistischeres Terrain
                    float noiseHeight = perlinNoise2D(ix / NOISE_SCALE, iy / NOISE_SCALE) * NOISE_AMPLITUDE * roughnessAmount;
                    float randomHeight = (llFrand(1.0) * 2.0 - 1.0) * maxHeightChange * roughnessAmount * 0.3;
                    
                    float heightChange = (noiseHeight * 0.7 + randomHeight * 0.3);
    ...
    osTerrainFlush();
    llOwnerSay("✅ Region wurde erfolgreich mit Perlin-Noise aufgeraut!");
}

// ===== VERBESSERTE PFLANZEN-FUNKTION =====
plantOneTree()
{
    ...
    // ===== GEWICHTETE AUSWAHL NACH DEUTSCHEN WALDANTEILEN =====
    list verfuegbareObjekte = [];
    integer listLength = llGetListLength(ALLE_OBJEKTE) / 7;
    integer i;
    
    // Gewichtete Auswahl basierend auf Prioritäten (deutsche Waldanteile)
    ...
    // Gesamtgewicht berechnen (alle Prioritäten)
    ...
    // Zufällige Auswahl basierend auf Gewicht (deutsche Waldanteile)
...
default
{
    ...
    listen(integer channel, string name, key id, string message)
    {
        ...
        else if (message == "🏔️ Einebnen")
        {
            ...
            llDialog(id, "✅ Region wurde mit Perlin-Noise generiert.", ["OK"], MENU_CHANNEL);
        }
        else if (message == "🔄 Aufrauhen")
        {
            ...
            llDialog(id, "✅ Region wurde mit Perlin-Noise aufgeraut.", ["OK"], MENU_CHANNEL);
        }
